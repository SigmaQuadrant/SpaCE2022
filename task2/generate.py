    ...
    with torch.no_grad():
        for idx, batch_sample in enumerate(dataloader):
            ...
            for ind in range(0, 12, 2):
                if result[idx][1][ind] > result[idx][1][ind + 1]:
                    result[idx][1][ind], result[idx][1][ind + 1] = result[idx][1][ind + 1], result[idx][1][ind]
...
if __name__ == '__main__':
    ...
    with open(config.test_dir, 'r') as fr:
        ...
        for idx, item in enumerate(jsonlines.Reader(fr)):
            ...
            items.append({'qid': qid, 'context': context, 'reasons': reason})
